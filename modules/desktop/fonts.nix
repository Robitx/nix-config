...

{
  options.desktop.fonts = {
    enable = lib.mkEnableOption "custom font configuration";
    
    enableNerdFonts = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Whether to install Nerd Fonts collection";
    };
  };
  ...
  config = lib.mkIf config.desktop.fonts.enable {
    fonts = {
      packages = with pkgs; [
        ...
        fira-code
        jetbrains-mono
      ] ++ lib.optionals config.desktop.fonts.enableNerdFonts [
